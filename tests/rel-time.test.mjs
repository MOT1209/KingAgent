...
test('inside the week it names the day, past it a date', () => {
  ...
  // the weekday comes from the user's own locale, so it can be 'Thu' as easily
  // as 'Do' — anything that is letters (and nothing else) is a day name
  assert.match(weekday, /^[A-Za-z]{2,}\.?$/, `expected a weekday, got ${weekday}`);
