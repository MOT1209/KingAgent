...
test('findOnDisk locates a binary the shell never mentioned', async () => {
  // The login-shell probe only names agent CLIs; walking the real PATH for the
  // running process itself proves the fallback finds a binary on its own.
  const bin = path.basename(process.execPath, path.extname(process.execPath));
  const found = await findOnDisk(bin, { home: os.homedir(), env: process.env, platform: process.platform });
  assert.ok(found, `did not find ${bin} anywhere on PATH`);
  assert.equal(path.basename(found), path.basename(process.execPath));
});
