...
test('a real zsh reports the failing stage of a pipeline', { skip: !HAS_ZSH && 'no /bin/zsh on this machine' }, () => {
...
test('nothing of the sequence is left visible in the output', { skip: !HAS_ZSH && 'no /bin/zsh on this machine' }, () => {
...
test('curl | bash reports zero even when curl failed', { skip: !HAS_ZSH && 'no /bin/zsh on this machine' }, () => {
...
test('a one-shot is spawned with its command, so nothing is typed', () => {
  // oneShotArgs is a pure builder taking platform; pin the shipped mac shape
  // regardless of the host this suite happens to run on.
...
test('spawned for real: the code is reported and the output survives', { skip: !HAS_ZSH && 'no /bin/zsh on this machine' }, () => {
