...
// elevated shell. Whether they exist changes what some tests can prove, so the
// probe is done once, here, where the links are built.
let symlinks = true;
...
test('a symlinked skill is a real skill, and a dangling one is shown as broken', { skip: !symlinks && 'session cannot create symlinks' }, () => {
  const items = scanLibrary({ projectPath: project, homeDir: home });
...
test('createItem scaffolds a claude project agent and refuses overwrite', () => {
  ...
  assert.ok(res.ok);
  assert.match(res.filePath, /[\\/]\.claude[\\/]agents[\\/]my-new-agent\.md$/);
...
test('createItem scaffolds a skill in the project\'s own folder, and an opencode agent', () => {
  ...
  assert.ok(sk.ok);
  assert.match(sk.filePath, /[\\/]skills[\\/]cool-skill[\\/]SKILL\.md$/, sk.filePath);
  ...
  assert.ok(oc.ok);
  assert.match(oc.filePath, /[\\/]\.config[\\/]opencode[\\/]agent[\\/]oc-agent\.md$/);
...
test('"Use here" copies a skill into the project\'s own folder, -copy on collision', () => {
  ...
  assert.ok(one.ok);
  assert.match(one.filePath, /[\\/]skills[\\/]tdd[\\/]SKILL\.md$/);
  ...
  assert.ok(two.ok);
  assert.match(two.filePath, /[\\/]skills[\\/]tdd-copy[\\/]SKILL\.md$/);
});
...
test('"Use here" refuses a skill whose files are gone', { skip: !symlinks && 'session cannot create symlinks' }, () => {
  const items = scanLibrary({ projectPath: project, homeDir: home });
...
test('duplicateItem copies a plugin agent file into the project', () => {
  ...
  assert.ok(res.ok);
  assert.match(res.filePath, /[\\/]\.claude[\\/]agents[\\/]critic\.md$/);
});
