...
// Everything is matched in forward-slash, drive-stripped form, so the win32
// prefix path.resolve adds does not hide a symlink that this stub was built to
// see.
const realpath = (links = {}) => {
  const norm = (p) => posix(p).replace(/^[A-Za-z]:/, '');
  const resolve = (p) => {
    const pn = norm(p);
    ...
    for (const k of keys) {
      const kn = norm(k);
      if (links[k] === null && pn === kn) { const e = new Error('ENOENT'); e.code = 'ENOENT'; throw e; }
      if (pn === kn) return resolve(links[k]);
      if (pn.startsWith(kn + '/')) return resolve(links[k] + pn.slice(kn.length));
    }
    return pn;
  };
