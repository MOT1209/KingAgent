...
// linkNative exercises real symlinks, and Windows needs Developer Mode or an
// elevated shell to make one. Probe once; the two tests that depend on the
// links being real skip when this session cannot grant that.
const canLink = await symlinksSupported();
...
test('linkNative gives Claude a relative link into the one real folder', { skip: !canLink && 'session cannot create symlinks' }, () => {
  const dir = tmp();
...
test('linkNative is idempotent and drops links whose skill has gone', { skip: !canLink && 'session cannot create symlinks' }, () => {
  const dir = tmp();
