...
test('detectServices merges catalog names, flags strangers as custom, reports scope and platform', () => {
  const io = memIo({
    [p('/proj/.mcp.json')]: JSON.stringify({ mcpServers: { notion: { command: 'npx' }, wiki: { command: 'node' } } }),
    [p('/home/u/.config/opencode/opencode.json')]: JSON.stringify({ mcp: { notion: { type: 'local' } } }),
  });
  const out = detectServices({ projectPath: p('/proj'), home: p('/home/u'), io });
