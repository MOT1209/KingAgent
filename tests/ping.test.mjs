...
const { pingPlan, sendPing } = require('../src/main/ping.js');

// KingAgent ships with PING_URL unset (a renamed fork must not count its users
// on the upstream counter), so a plan only exists when the override points the
// mechanism at a real endpoint. The override below is that endpoint.
const PING_OVERRIDE = 'http://localhost:8788/api/ping';
...
test('first launch mints an id and flags first', () => {
  ...
  assert.equal(plan.url, PING_OVERRIDE);
...
test('a dev run sends nothing', () => {
  // Unpackaged = someone building the app without an endpoint of their own;
  // their launches must not count as users.
  assert.equal(pingPlan({ ...base(), isPackaged: false, env: {} }), null);
...
test('sendPing posts the payload and persists a minted id', async () => {
  ...
  assert.equal(calls[0].url, PING_OVERRIDE);
