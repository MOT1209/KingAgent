...
// on a kingagent-doc source". Plus the served policy must forbid the network, or a
...
function iframeSandboxes(src) {
  ...
  while ((m = re.exec(src))) {
    ...
    const source = m[3] === 'docUrl' || /docUrl\(/.test(m[2]) ? 'kingagent-doc'
      : m[3] ? 'file' : 'srcdoc';
  ...
  while ((m = re2.exec(src))) {
    out.push({ sandbox: m[1].trim(), source: m[2] === 'docUrl' ? 'kingagent-doc' : 'file' });
  }
  return out;
}
...
test('allow-same-origin appears only on a kingagent-doc source', () => {
...
test('the doc scheme is registered standard and secure (gives its own origin)', () => {
  assert.match(main, /scheme:\s*['"]kingagent-doc['"]/);
