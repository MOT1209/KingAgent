...
// Keys fold to forward slashes so platform-joined paths hit the literals.
function memIo(present = []) {
  const files = new Set(present.map(posix));
  return {
    files,
    exists: (p) => files.has(posix(p)),
    mkdir: () => {},
    write: (p) => { files.add(posix(p)); },
    rename: (a, b) => { files.delete(posix(a)); files.add(posix(b)); },
