...
// under its final name until it is complete. Keys are folded to forward slashes
// so platform-joined paths compare against the literals written here.
function memIo() {
  const files = new Set();
  const log = [];
  return {
    files, log,
    exists: (p) => files.has(posix(p)),
    mkdir: () => {},
    write: (p) => { files.add(posix(p)); log.push(['write', p]); },
    rename: (a, b) => { files.delete(posix(a)); files.add(posix(b)); log.push(['rename', a, b]); },
    remove: (p) => { for (const f of [...files]) if (f === posix(p) || f.startsWith(posix(p) + '/')) files.delete(f); },
  };
}
function okFetch() {
  const calls = [];
