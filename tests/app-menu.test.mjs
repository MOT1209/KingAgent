...
const { buildMenuTemplate, menuRoles, menuItems, LINKS, COMMANDS, REPO } = require('../src/main/app-menu.js');
...
test('six menus on macOS, in the order a Mac user reads them', () => {
  ...
  assert.deepEqual(labels, ['KingAgent', 'File', 'Edit', 'View', 'Window', 'Help']);
});
...
test('About KingAgent opens KingAgent’s own pane, not the grey macOS panel', () => {
  const sent = [];
  const kingagent = build({ send: (c) => sent.push(c) }).find((m) => m.label === 'KingAgent');
  assert.ok(kingagent, 'no app submenu');
  assert.ok(!kingagent.submenu.some((i) => i.role === 'about'), 'role:about is the stock panel');
  const about = kingagent.submenu.find((i) => i.label === 'About KingAgent');
  about.click();
...
// Six items, five destinations: the repo is opened by both KingAgent on GitHub
// and ★ Star KingAgent, which is deliberate. KingAgent Docs opens the same
// repo's readme, so the raw url count is two for the repo plus one each for the
// other four links. One item is "read the source", the other is an ask, and
// GitHub has no separate page for either.
...
test('help-menu links all point at the repo, and none carries a utm', () => {
  for (const key of Object.keys(LINKS)) {
    assert.ok(LINKS[key].startsWith(REPO), `${key} leaves the repo`);
