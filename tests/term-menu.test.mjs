...
test('a file offers open, reveal and copy', () => {
  ...
  assert.deepEqual(labels(items), ['Open', reveal, 'Copy path']);
});
...
test('a directory does not offer Open — ⌘click reveals it, and so does this', () => {
  ...
  assert.deepEqual(labels(items), [reveal, 'Copy path']);
});
...
test('the shortcuts advertised are the ones that exist', () => {
  ...
  assert.equal(byLabel(reveal).kb, '⌥⌘click');
  ...
  assert.equal(dir.find((i) => i !== '-' && i.label === reveal).kb, '⌘click');
});
