...
test('with nothing scanned it behaves exactly as it did before', () => {
  forgetBins();
  if (IS_WINDOWS) {
    // the win32 floor is claude.exe from the installers claudeCandidates names
    const winHome = p('/Users/x');
    const native = p('/Users/x/.local/bin/claude.exe');
    assert.equal(resolveClaudeExecutable({ home: winHome, env: {}, exists: only(native) }), native);
    const local = p('/Users/x/.claude/local/claude.exe');
    assert.equal(resolveClaudeExecutable({ home: winHome, env: {}, exists: only(local) }), local);
    assert.equal(resolveClaudeExecutable({ home: winHome, env: {}, exists: () => false }), null);
  } else {
...
test('a remembered path that no longer exists falls through', () => {
  ...
  rememberBins([{ id: 'claude', found: true, path: NVM }]);
  if (IS_WINDOWS) {
    const floor = p('/Users/x/.local/bin/claude.exe');
    const exe = resolveClaudeExecutable({ home: p('/Users/x'), env: {}, exists: only(floor) });
    assert.equal(exe, floor);
  } else {
