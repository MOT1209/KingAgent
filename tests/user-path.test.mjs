...
test('userPath asks the shell once and reuses the answer', async () => {
  ...
  const a = await userPath({ exec, env: { PATH: LAUNCHD }, platform: 'darwin' });
  const b = await userPath({ exec, env: { PATH: LAUNCHD }, platform: 'darwin' });
...
test('a shell that throws degrades to the PATH we already had, not to nothing', async () => {
  resetForTests();
  const out = await userPath({ exec: async () => { throw new Error('no tty'); }, env: { PATH: LAUNCHD }, platform: 'darwin' });
  assert.equal(out, LAUNCHD);
});

test('a shell that hangs and returns empty still yields a usable PATH', async () => {
  resetForTests();
  const out = await userPath({ exec: async () => '', env: { PATH: LAUNCHD }, platform: 'darwin' });
  assert.equal(out, LAUNCHD);
});
...
test('after an install the shell is asked again', async () => {
  ...
  assert.equal(await userPath({ exec, env: { PATH: '' }, platform: 'darwin' }), '/usr/bin');

  refreshUserPath();

  assert.equal(await userPath({ exec, env: { PATH: '' }, platform: 'darwin' }), '/Users/x/.local/bin:/usr/bin');
