...
test('shellQuote leaves plain arguments alone and neutralises the rest', () => {
  ...
  assert.equal(shellQuote('*'), "'*'");
  // the one character single quotes cannot carry, closed and reopened on POSIX;
  // PowerShell doubles the quote inside its own literal — the platform dialect
  assert.equal(shellQuote("Cal's button"), IS_WINDOWS ? "'Cal''s button'" : "'Cal'\\''s button'");
});
