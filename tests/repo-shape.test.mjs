...
// These rules police what a Git checkout publishes, so they only mean something
// when this folder *is* a repository root. During development the folder can
// live inside a larger monorepo where `git ls-files` answers for the wrong
// tree; the check below is the first thing that tells the difference.
let tracked = [];
...
// private notes.
test('everything a stranger needs is still here', { skip: notStandalone }, () => {
...
test('no tracked file carries anything shaped like a credential', { skip: notStandalone }, () => {
  const PATTERNS = [
