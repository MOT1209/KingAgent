...
test('browserFileUrl accepts existing absolute html paths', () => {
  // The POSIX literals are drive-relative on Windows: /Users/cal resolves onto
  // the session drive (C:), which is what file:// then carries.
  const base = IS_WINDOWS ? 'file:///' + path.parse(process.cwd()).root[0] + ':' : 'file://';
  assert.equal(
    browserFileUrl('/Users/cal/My Site/index.html', { statSync: fileStat }),
    base + '/Users/cal/My%20Site/index.html',
  );
  assert.equal(
