...
// with the host's separator, so keys and lookups are folded to posix form and
...
function memIo(seed = {}) {
  ...
  return {
    ...
    remove: (f) => { delete files[posix(f)]; },
    list: (dir) => Object.keys(files).filter((f) => f.startsWith(posix(dir) + '/')).map((f) => f.slice(posix(dir).length + 1)).filter((r) => !r.includes('/')),
    files,
  };
}
...
test('codex copy is TOML with the prompt as developer_instructions', () => {
  ...
  assert.match(text, /^# made by KingAgent from agents\/release-scribe\.md/);
...
test('a master reaches grok as project markdown', () => {
  ...
  assert.match(copy, /made by KingAgent from agents\/scribe\.md/);
  assert.match(copy, /^---\n/);
...
test('deleting a master sweeps its grok copy too', () => {
  ...
  assert.ok(removed.some((f) => posix(f) === '/p/.grok/agents/scribe.md'));
});
