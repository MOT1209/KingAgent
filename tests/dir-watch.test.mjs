...
function harness(opts = {}) {
  ...
  const w = createDirWatch({
    watch,
    // dirs and files are joined with the host's separator; fold them back so
    // the assertions keep their POSIX literals
    onChange: (dir, files) => {
      const d = posix(dir);
      changed.push(d);
      events.push({ dir: d, files: files === null ? null : files.map(posix) });
    },
    setTimeoutFn: setT,
