...
// Keys fold to forward slashes: the src joins the cache path with this
// platform's separator, and the literals below are the POSIX spelling of it.
const stubFs = (files) => ({
  readFileSync: (p) => {
    const key = posix(p);
  ...
  existsSync: (p) => posix(p) in files,
});

test('a complete staged download is found', () => {
  const io = stubFs({
    '/c/pending/update-info.json': '{"fileName":"KingAgent-arm64.zip","sha512":"x"}',
    '/c/pending/KingAgent-arm64.zip': 'bytes',
  });
...
test('info without the file it names is not a staged download', () => {
  ...
  const io = stubFs({ '/c/pending/update-info.json': '{"fileName":"KingAgent-arm64.zip"}' });
