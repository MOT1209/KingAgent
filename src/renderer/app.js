...
import { isFile as isFilePanel, isSession as isSessionPanel, ownerFor, groupRail, keep as keepFile, splitAfter, splitLayout } from './desk-view.mjs';
...
// onStarClick is a thunk (not the plain REPO_URL value) because REPO_URL is
// declared later, in the About section — a closure only reads it at call
// time, once the star ask actually fires, well after the whole module (and
// REPO_URL) has finished loading.
...
async function swapDesk(info) {
  ...
  // flushPanels() itself clears the pending debounce before writing — the
  // clearTimeout that used to sit here was fully redundant with the one
  // inside it, and saveTimer is private to panel-lifecycle.mjs now anyway.
