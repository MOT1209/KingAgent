...
// "Reveal in Finder" is macOS's phrase; Windows and Linux have File Explorer
// and no Finder. Same verb, the user's own noun — platform is a parameter so
// the shape stays testable without a running app.
function revealLabel(platform = process.platform) {
  return platform === 'darwin' ? 'Reveal in Finder' : 'Reveal in File Explorer';
}

export function termMenuItems({ kind, text, st, platform = process.platform }) {
  if (kind === 'url') {
    return [
  ...
  if (st.isDir) {
    return [
      { label: revealLabel(platform), kb: '⌘click' },
      '-',
