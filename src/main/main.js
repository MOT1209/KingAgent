...
// Windows groups every window of a process under one taskbar identity keyed by
// the AppUserModelId. Without this the pinned icon is a generic Electron one
// and jump-list state never attaches to the app. Harmless elsewhere.
if (process.platform === 'win32') {
  try { app.setAppUserModelId('ai.kingagent.app'); } catch (_) {}
}
...
function stagedUpdateWaiting() {
  try { return hasStagedFile(cacheDir()); } catch (_) { return false; }
}
...
ipcMain.handle('services:pickBundle', async (e) => {
  ...
  // A bundle is a zip; every OS ships an unpacker and none agree on its name,
  // so unzip.js asks each in its own dialect — /usr/bin/unzip on the Mac
  // (what this always did), Expand-Archive on Windows. Extract first into a
  // scratch spot named after the file, read the manifest,
  // then settle under the manifest's own name+version.
  const tmp = path.join(os.homedir(), APP_DATA_DIRNAME, 'bundles', '.unpacking-' + Date.now());
  const unzipErr = await unzip({ file, dir: tmp });
  if (unzipErr) return { ok: false, error: 'Could not unpack it: ' + unzipErr };
  try {
    ...
    const dir = path.join(os.homedir(), APP_DATA_DIRNAME, 'bundles', bundleSlug(parsed.manifest));
...
ipcMain.handle('folder:make', async (e) => {
  ...
  const res = await dialog.showSaveDialog(parent, {
    title: 'Where should KingAgent work?',
    defaultPath: path.join(app.getPath('documents'), 'KingAgent'),
    buttonLabel: 'Create',
...
ipcMain.handle('term:create', async (e, { id, cwd, cols, rows, kind, command, program, args, seed, cont, sid, acpSid, name, watchDone }) => {
  ...
  // The terminal is a platform decision, made once: pwsh → powershell → cmd
  // on Windows, the user's own shell on the Mac. See terminal.js.
  ...
  if (kind === 'claude') {
    ...
    // shellQuote is platform-aware (POSIX or PowerShell dialect — see
    // claude-args.js); the map gets this platform so the typed line parses
    // in the shell it is about to land in.
    ...
    let typed = resolveRunCommand(withSpawnFlags(command), process.platform);
    ...
    if (agent) {
      if (cont && acpSid) {
        ...
        if (resume) { typed = resolveRunCommand(withSpawnFlags(resume), process.platform); storeWatch = { agent, sid: acpSid }; }
      } else if (!acpSid) discoverAgent = agent;
    }

    if (watchDone) { spawnArgs = oneShotArgs(shellPath, typed, process.platform); echoLine = command; }
