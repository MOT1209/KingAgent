...
// could not.
//
// Windows gets its own shape. There is no `exec`, and the sentinel cannot be
// printf: PowerShell's `$?` is a boolean and its printf does not expand
// `\033`. So the command runs under `powershell -NoExit -Command`, the exit
// code comes from `$LASTEXITCODE` (natives only — which is exactly the case
// here), and the same OSC 1337 bytes are emitted with [char]27/[char]7. The
// shell's profile is NOT suppressed: an install tile needs the user's PATH.
// -NoExit stands in for `exec shell -i` — the tile stays a live prompt that
// can run whatever was just installed.
