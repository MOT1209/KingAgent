...
function createReviewProfile({ argv, normalPath, packaged, reviewBuild = false, homePath = os.homedir() }) {
  ...
  // The persistent review desk sits beside the real app's own profile, named
  // per platform: ~/Library/Application Support on macOS, %APPDATA% on
  // Windows (what Electron itself uses for userData). Hard-coding the mac
  // path put a Library folder inside whatever HOME a Windows reviewer had.
  const reviewHome = process.platform === 'win32'
  ...
  const directory = explicit ? path.resolve(explicit)
    : persistent ? reviewHome
      : owned ? fs.mkdtempSync(path.join(os.tmpdir(), 'kingagent-review-'))
