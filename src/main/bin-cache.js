...
//
// KingAgent answers "is this agent installed" properly in exactly one place:
...
// login PATH captured once at app start, so an agent installed *inside* KingAgent
...
const fs = require('fs');
const os = require('os');
const { claudeCandidates, psQuote } = require('./platform.js');
...
//
// The head is quoted in the shell's own dialect — POSIX single quotes with
// the closed-escape trick, or PowerShell's doubled-quote literal — because a
// path with spaces arrives here on both platforms. `platform` is a parameter
// so the decision is testable from either machine.
function resolveRunCommand(command, platform = process.platform) {
  ...
  const head = /[^\w@%+=:,./\\-]/.test(found)
    ? (platform === 'win32' ? psQuote(found) : `'${found.replace(/'/g, `'\\''`)}'`)
    : found;
  return head + (m[2] || '');
}
...
// ---- spawn flags -----------------------------------------------------------
// Flags KingAgent always adds when it spawns a given agent, as opposed to anything
// the user or the resume path asked for.
//
// grok is the only entry and --minimal is the reason the table exists: without
// it grok paints a full-screen TUI over the KingAgent theme instead of printing
