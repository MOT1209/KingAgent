...
// The install commands the setup sheet shows. Unix gets the curl-pipe-bash
// lines the official docs print; Windows gets the npm/winget routes the same
// docs offer, because `curl … | bash` is not a sentence a Windows shell can
// act on. `available` tells the renderer whether the line it is showing can
// run on this machine — a mac line on Windows is a prompt to read, not a
// button to press.
...
const KNOWN_AGENTS = [
  ...
  { id: 'grok', name: 'Grok', bin: 'grok', kind: 'run',
    ...
    installWin: 'npm install -g @vibe-kit/grok-cli',
  ...
  { id: 'antigravity', name: 'Antigravity', bin: 'agy', kind: 'run',
    ...
    installWin: null,   // no documented Windows route yet — shown as a doc link, not a command
  ...
  { id: 'hermes', name: 'Hermes', bin: 'hermes', kind: 'run',
    ...
    installWin: null,   // no documented Windows route yet
...
async function detectAgents({ exec = shellWhich, home = os.homedir() } = {}) {
  ...
  return Promise.all(KNOWN_AGENTS.map(async (a) => {
    ...
    // The command this platform can actually run, and whether there is one.
    // A null installWin is an honest "the docs offer nothing here", not an
    // oversight — the renderer shows the doc link instead of a dead command.
    const installFor = platform === 'win32' ? a.installWin : a.install;
    return { ...a, found: !!p, path: p, pathShort: shortHome(p, home), configFile,
      installCommand: installFor || a.install,
      installAvailable: !!installFor };
  }));
}
