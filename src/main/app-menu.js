...
function buildMenuTemplate({
  open, send, newWindow, platform = process.platform, name = 'KingAgent', theme = 'paper', recents = [],
} = {}) {
  ...
  const fileSubmenu = [
    ...
    cmd(platform === 'darwin' ? 'Reveal in Finder' : 'Reveal in File Explorer', 'reveal'),
    SEP,
  ...
  const helpSubmenu = [
    ...
    SEP,
    link('KingAgent on GitHub', LINKS.repo),
    link('★ Star KingAgent', LINKS.repo),
    link('Report an Issue', LINKS.issue),
    link('Release Notes', LINKS.releases),
    SEP,
    link('License', LINKS.terms),
    ...(mac ? [] : [SEP, cmd('About KingAgent', 'about'), cmd('Check for Updates', 'update-check')]),
  ];
...
function installAppMenu({ Menu, shell, app: electronApp, send, newWindow, theme, recents }) {
  const template = buildMenuTemplate({
    ...
    send,
    newWindow,
    name: (electronApp && electronApp.name) || 'KingAgent',
    theme,
    recents,
  });
