...
// --user-data-dir is not optional here: with the user's own Chrome already
// running, a headless launch against the default profile blocks on the profile
// lock and never returns a screenshot. An isolated throwaway profile makes the
// render deterministic whether or not a browser is open.
execFileSync(CHROME, ['--headless=new', '--disable-gpu', '--no-sandbox', '--hide-scrollbars', '--force-device-scale-factor=2',
  '--user-data-dir=' + path.join(os.tmpdir(), 'brand-render-profile'),
