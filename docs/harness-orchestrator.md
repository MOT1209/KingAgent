...
> **Note:** this document describes the harness-orchestrator layer
> (`src/core/harness-orchestrator/`), a second control-plane that coexists
> with the Phase 3 orchestrator (`src/core/orchestrator/`, documented in
> [orchestrator.md](./orchestrator.md)) rather than replacing it — the two
> were built independently and reconciled side by side. See the module
> comment at the top of `src/core/index.js` for why, and
> `platform.harnessOrchestrator` / `platform.orchestrator` for how each is
> reached.
...
User
 │
 ▼
Orchestrator                src/core/harness-orchestrator/orchestrator.js
 │
